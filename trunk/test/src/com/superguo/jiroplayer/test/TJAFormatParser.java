...
public final class TJAFormatParser
{
	...
	public void parse(TJAFormat format, BufferedReader reader) throws IOException
	{
				...
				mLine = reader.readLine())
		{
			...
			else if ((colonPos = mLine.indexOf(':')) >= 0)
			{
				try
				{
					...
					setHeader(	mLine.substring(0, colonPos).trim(),
								mLine.substring(colonPos+1).trim() );
				}
		...
		if (	mParsingCourse.iNotationSingle != null ||
				mParsingCourse.iNotationP1 != null &&
				mParsingCourse.iNotationP2 != null)
		{
			emitCourse();
		}
		
		if (mParsingCourse.iNotationP1 != null &&
			mParsingCourse.iNotationP2 == null)
		{
		...
		if (mParsedCourses.size()==0)
			...
			format.courses = mParsedCourses.toArray(new TJACourse[mParsedCourses.size()]);
	}
	...
	{
		if (mParsingNotes.position()<=0) return;

		TJACommand cmd = new TJACommand(TJAFormat.COMMAND_TYPE_NOTE);
		cmd.iArgs = new int[mParsingNotes.position()];
		System.arraycopy(
				mParsingNotes.array(), 0, cmd.iArgs, 0,
	...
	private void parseCommandOfCurrentLine() {
		...
		// judge if started
		if (!mIsStarted &&
			!mLine.startsWith("#START") && 
			!mLine.equals("#BMSCROLL") &&
			!mLine.equals("#HBSCROLL"))
		{
		...
		if (mLine.startsWith("#START"))
		{
			...
			else if	(fields.length==2)
			{
				...
				else if (fields.length==2 && fields[1].equals("P2"))
				{
					...
					if (mParsingCourse.iNotationP2!=null)
		...
		{
			if (mIsGoGoStarted) throwEx("Already exist #GOGOSTART before #GOGOEND");
			mIsGoGoStarted = true;
			mParsedCommands.add(new TJACommand(TJAFormat.COMMAND_TYPE_GOGOSTART));
		}
		else if (mLine.equals("#GOGOEND"))
		{
			if (!mIsGoGoStarted) throwEx("Missing #GOGOSTART before #GOGOEND");
		...
		else if (mLine.equals("#N"))
		{
			...
			// fill back
			if (0!=mCommandOfStartedBranch.iArgs[3])
				throwEx("Duplicated #N");
			mCommandOfStartedBranch.iArgs[3] = mParsedCommands.size()-1;
		}
		else if (mLine.equals("#E"))
		{
			...
			// fill back
			if (0!=mCommandOfStartedBranch.iArgs[4])
				throwEx("Duplicated #E");
			mCommandOfStartedBranch.iArgs[4] = mParsedCommands.size()-1;
		}
		else if (mLine.equals("#M"))
		{
			...
			// fill back
			if (0!=mCommandOfStartedBranch.iArgs[5])
				throwEx("Duplicated #M");
			mCommandOfStartedBranch.iArgs[5] = mParsedCommands.size()-1;
		}
		else if (mLine.equals("#BRANCHEND"))
		{
			emitBranch();
			mParsedCommands.add(new TJACommand(TJAFormat.COMMAND_TYPE_BRANCHEND));
			mIsBranchStarted = false;
		}
		else if (mLine.equals("#LEVELHOLD"))
		{
			mParsedCommands.add(new TJACommand(TJAFormat.COMMAND_TYPE_LEVELHOLD));
		}
	}
	...
	private void emitNotation()
	{
		...
		{
			TJACommand[] notation = mParsedCommands.toArray(
					new TJACommand[mParsedCommands.size()]);
			...
			if (!mIsParsingDouble)
				mParsingCourse.iNotationSingle = notation;
			else if (!mIsParsingP2)
			{
				mParsingCourse.iNotationP1 = notation;
				mIsParsingP2 = false;
			}
			else
			{
				mParsingCourse.iNotationP2 = notation;
	...
	private void setHeader(String name, String value)
	{
		...
		{
			mParsingCourse.iLevel = Integer.parseInt(value);
			if (mParsingCourse.iLevel<1 || mParsingCourse.iLevel>12)
				throwEx("Bad level");
		}
		else if (name.equals("BPM"))
		{
			mParsingCourse.iBPM = mBPM = Float.parseFloat(value);
			if (mParsingCourse.iBPM < 50 || mParsingCourse.iBPM > 250)
		...
		else if (name.equals("BALLOON"))
		{
			...
			mParsingCourse.iBalloon = new int[balloons.length];
			for (int i=0; i<mParsingCourse.iBalloon.length; ++i)
				mParsingCourse.iBalloon[i] = Integer.parseInt(balloons[i].trim()); 
		}
		...
		{
			mParsingCourse.iScoreInit = Integer.parseInt(value);
			if (mParsingCourse.iScoreInit<0)
				throwEx("SCOREINIT must be positive");
		}
		else if (name.equals("SCOREDIFF"))
		{
			mParsingCourse.iScoreDiff = Integer.parseInt(value);
			if (mParsingCourse.iScoreDiff<0)
	...
	{
		if (mCommandOfStartedBranch.iArgs[3]==0)
			throwEx("Missing #N");
		else if (mCommandOfStartedBranch.iArgs[4]==0)
			throwEx("Missing #E");
		else if (mCommandOfStartedBranch.iArgs[5]==0)
			throwEx("Missing #M");

		mCommandOfStartedBranch.iArgs[6] = mParsedCommands.size();
	}
}